...
#     count = array.count {|ele| block.call(ele)}
#     count == n
# end
...
#     arr.inject(value) {|acc, el| el.call(acc)}
# end
...
def proc_suffix(sent, hash)
    result = []
    sent1 = sent.split
    sent1.each do |ele| # [dog, cat]
        new_word = ele # what does it looks like
        hash.each do |procs,v|
                new_word += v if procs.call(ele) # why cant new_word = ele +=v if procs.call(ele)
            end
            result << new_word
        end
    result.join(" ")
